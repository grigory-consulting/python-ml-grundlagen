...
+ Eine lauffähige **conda-Umgebung** mit Jupyter in VS Code und das Kursrepo auf Ihrem Gerät
...
| Teil | Thema |
|------|-------|
| 1 | Arbeitsumgebung einrichten: conda, Jupyter in VS Code, Git |
| 2 | Python-Grundlagen |
| 3 | NumPy und pandas |
...
Eine conda-Umgebung, das Kursrepo und VS Code mit Jupyter: danach läuft jedes Notebook des Kurses auf Ihrem Gerät.

--

## Was Sie in diesem Teil lernen

+ Eine eigene conda-Umgebung `pyml` anlegen und aktivieren
+ Die Kurspakete mit `pip` installieren
+ Das Kursrepo mit Git klonen und aktuell halten
+ In VS Code ein Notebook öffnen und den Kernel `pyml` wählen
+ Notebooks sicher bedienen: Zellen, Tastenkürzel, Reihenfolge, Hilfe

--

## Was auf Ihrem Gerät bereits installiert ist

| Werkzeug     | Wofür                                                                   |
|--------------|-------------------------------------------------------------------------|
| **Python**   | Der Interpreter: führt Ihren Code aus                                    |
| **Anaconda** | Python-Distribution, bringt `conda` mit: Paket- und Umgebungsverwaltung |
| **VS Code**  | Editor mit Erweiterungen für Python, Notebooks, Debugging und Git        |
| **Git**      | Versionskontrolle: holt das Kursrepo auf Ihr Gerät                       |

Was noch fehlt, richten wir jetzt gemeinsam ein:

+ eine eigene Umgebung für den Kurs
+ die Pakete für Datenanalyse und Machine Learning
+ das Kursrepo mit Notebooks und Daten

--

## Anaconda Prompt öffnen

Startmenü → **Anaconda Prompt**. Vor dem Pfad steht die aktive Umgebung: `(base)`.
...
Eine conda-Umgebung ist ein abgeschlossener Ordner mit eigenem Python und eigenen Paketen.

+ `base` bleibt unverändert und damit funktionsfähig
+ Jedes Projekt bekommt genau die Pakete, die es braucht
+ Versionskonflikte zwischen zwei Projekten bleiben aus
+ Geht etwas schief, löschen Sie die Umgebung und legen sie neu an: `conda env remove -n pyml`
+ Mit einer `environment.yml` bauen Kolleginnen und Kollegen dieselbe Umgebung nach

<div class="fragment">

> [!tip]
> Eine Umgebung pro Projekt. In `base` wird nichts installiert.
...
```text
conda create -n pyml python=3.12
conda activate pyml
```

+ `-n pyml` gibt der Umgebung ihren Namen, `python=3.12` legt die Python-Version fest
+ conda zeigt den Installationsplan und fragt nach: mit `y` bestätigen
+ Nach dem Aktivieren wechselt der Prompt von `(base)` zu `(pyml)`
+ `conda env list` zeigt alle Umgebungen, die aktive ist mit `*` markiert
+ Zurück in die Grundumgebung: `conda deactivate`

<div class="fragment">

> [!warning]
> Steht vorn nicht `(pyml)`, landen alle folgenden Installationen in der falschen Umgebung.
...
In der aktiven Umgebung `(pyml)`, aufgeteilt auf zwei Befehle:
...
| Pakete                       | Wofür                                     |
|------------------------------|-------------------------------------------|
| `numpy`, `pandas`            | Rechnen mit Arrays, Tabellen auswerten    |
| `matplotlib`, `seaborn`      | Diagramme                                 |
| `scikit-learn`               | Machine Learning                          |
| `torch`, `torchvision`       | Deep Learning                             |
| `datasets`                   | Datensätze von Hugging Face laden         |
| `jupyter`, `ipykernel`       | Notebooks und die Verbindung zu VS Code   |

> [!important]
> Die Netzfreigaben für die Paketquellen gelten nur während der Schulung. Installieren Sie deshalb jetzt alle Pakete vollständig.

--

<!-- .slide: class="smaller" -->
## Git in drei Befehlen

Ein Repository ist ein Projektordner mit allen Dateien und einem versteckten Ordner `.git`, in dem Git die gesamte Historie speichert.

| Befehl            | Was er tut                                                             |
|-------------------|------------------------------------------------------------------------|
| `git clone <url>` | Kopiert das Repository samt Historie auf Ihr Gerät (einmalig)          |
| `git pull`        | Holt neue Stände des Kursrepos nach (im Repo-Ordner ausführen)         |
| `git status`      | Zeigt, welche Dateien Sie lokal geändert haben                         |

```text
cd %USERPROFILE%\Documents
git clone https://github.com/grigory-consulting/python-ml-grundlagen.git
cd python-ml-grundlagen
git status
```
...
`requirements.txt` listet die Pakete zeilenweise auf.

</div>
<div style="flex: 50">

**conda: Umgebung komplett aus Datei**
...
`environment.yml` legt Name, Python-Version und Pakete fest.

</div>
</div>

> [!tip]
> Eine solche Datei im Projektordner macht Ihre Umgebung für andere nachbaubar.

--

## VS Code: Ordner öffnen und Erweiterungen installieren

+ VS Code starten, dann **Datei → Ordner öffnen** und den Ordner `python-ml-grundlagen` wählen
+ Alternativ aus dem Prompt im Repo-Ordner: `code .`
+ Die Sicherheitsabfrage „Vertrauen Sie den Autoren?" bestätigen
+ Seitenleiste **Erweiterungen** (`Strg+Umschalt+X`) öffnen und installieren:
    + **Python** (Microsoft)
    + **Jupyter** (Microsoft)
+ Links im Explorer sehen Sie jetzt die Dateien des Kursrepos
...
## VS Code: Kernel `pyml` wählen

Der Kernel ist der Python-Prozess, der die Zellen eines Notebooks ausführt. Er muss aus `pyml` kommen, sonst fehlen die Pakete.

+ Eine `.ipynb`-Datei im Explorer öffnen
+ Oben rechts auf **Kernel auswählen** klicken
+ **Python-Umgebungen** wählen, dann den Eintrag `pyml`
+ Oben rechts steht danach `pyml` mit der Python-Version
+ Für `.py`-Dateien: `Strg+Umschalt+P` → **Python: Interpreter auswählen** → `pyml`

<div class="fragment">

> [!warning]
> `ModuleNotFoundError: No module named 'pandas'` heißt fast immer: falscher Kernel gewählt.
...
<!-- .slide: class="smaller" -->
## Notebook bedienen: Zellen und Tastenkürzel

Ein Notebook besteht aus Zellen. **Code-Zellen** enthalten Python und zeigen ihr Ergebnis direkt darunter, **Markdown-Zellen** enthalten formatierten Text.

| Aktion                                        | Tastenkürzel              |
|-----------------------------------------------|---------------------------|
| Zelle ausführen, zur nächsten springen        | `Umschalt+Enter`          |
| Zelle ausführen, in der Zelle bleiben         | `Strg+Enter`              |
| Zelle bearbeiten / Bearbeiten verlassen       | `Enter` / `Esc`           |
| Neue Zelle darüber / darunter (nach `Esc`)    | `A` / `B`                 |
| Zelltyp Markdown / Code (nach `Esc`)          | `M` / `Y`                 |
| Zelle löschen (nach `Esc`)                    | `D`, `D`                  |

+ Der Wert des letzten Ausdrucks einer Zelle erscheint auch ohne `print`
+ Links neben der Zelle zeigt `[3]`, als wievielte sie ausgeführt wurde
+ Werkzeugleiste oben: **Alle ausführen**, **Neu starten**, **Variablen**
...
beitrag = beitrag * 2
print(beitrag)
# -> 200 beim ersten Ausführen, 400 beim zweiten, 800 beim dritten
```

+ Zelle 2 zweimal ausführen verändert das Ergebnis, obwohl der Code gleich bleibt
+ Eine gelöschte Zelle hinterlässt ihre Variablen im Kernel
+ Abhilfe: **Neu starten**, danach **Alle ausführen**

<div class="fragment">

> [!warning]
> Ein Notebook ist erst dann in Ordnung, wenn es nach „Neu starten" und „Alle ausführen" von oben nach unten fehlerfrei durchläuft.
...
```python
print?                 # Hilfe zu print in Jupyter
help(print)            # dasselbe in jeder Python-Umgebung

import math
help(math.sqrt)        # Hilfe zu einer Funktion aus einem Modul
print(print.__doc__)   # der Docstring als Text
```
...
## Prüfzelle: Versionen aller Pakete
...
+ Läuft die Zelle ohne Fehlermeldung durch, ist Ihre Umgebung vollständig
+ Die erste Zeile der Ausgabe nennt die Python-Version aus `pyml`
+ `ModuleNotFoundError`: Paket fehlt oder falscher Kernel. Kernel prüfen, dann `pip install <paket>` in `(pyml)`
+ Das Paket heißt `scikit-learn`, importiert wird es als `sklearn`

--

## Zusammenfassung

+ `conda create -n pyml python=3.12` und `conda activate pyml`: eine eigene Umgebung, `base` bleibt unberührt
+ Pakete kommen mit `pip install` in die aktive Umgebung, reproduzierbar über `requirements.txt` oder `environment.yml`
+ Drei Git-Befehle reichen: `git clone` einmalig, `git pull` für neue Stände, `git status` zur Kontrolle
+ In VS Code den Repo-Ordner öffnen und im Notebook den Kernel `pyml` wählen
+ Bei merkwürdigen Ergebnissen im Notebook: Kernel neu starten, alle Zellen ausführen
